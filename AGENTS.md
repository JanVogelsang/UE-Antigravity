...
## 5. Installing & Testing on a Target Project (Autonomous Agent Workflows)

These steps are designed for AI agents to verify that changes to the `UE-Antigravity` repository correctly integrate with a host game. We use the local project `C:\Users\janv1\Documents\Unreal Projects\tau-game` as our standard target project for this example. All procedures must be executed programmatically.
...
### 5.3 Verify Integration (Target Workspace Verification)
...
The `UE-Antigravity` repository contains an automated E2E test suite (`pytest`) that connects a mock client to the Python bridge and the Editor loopback server. You can run this directly from the `UE-Antigravity` root:

1. **Verify Editor Status**: Ensure the `tau-game` Unreal Engine Editor is running (listening on port `18777`).
2. **Execute Tests**: Run the automated test wrapper:
   ```powershell
