...
{
	FScopedTransaction Transaction(FText::FromString(TEXT("Antigravity Widget Action")));
	...
	if (ToolName == TEXT("macro_create_basic_ui_menu")) return ExecuteMacroCreateBasicUIMenu(Params, Result);
	if (ToolName == TEXT("capture_widget"))           return ExecuteCaptureWidget(Params, Result);

	Result.Errors.Add(FString::Printf(TEXT("Unknown Widget tool: '%s'. Supported: create_widget_blueprint, add_widget, set_widget_slot, set_widget_property, set_widget_font, set_widget_brush, bind_widget_event, remove_widget, get_widget_tree, compile_widget_blueprint, macro_create_basic_ui_menu, capture_widget"), *ToolName));
	return Result;
}
...
FAntigravityActionResult FAntigravityWidgetActions::ExecuteSetWidgetProperty(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	...
	// Special handling for Text property on TextBlock â€” use SetText for proper FText
...
FAntigravityActionResult FAntigravityWidgetActions::ExecuteBindWidgetEvent(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	...
	if (!EventGraph)
	{
		Result.Errors.Add(TEXT("Widget Blueprint has no EventGraph. This is unexpected â€” try recompiling the Widget Blueprint first."));
		return Result;
	}
