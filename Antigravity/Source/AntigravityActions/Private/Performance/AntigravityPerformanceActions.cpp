...
// ---------------------------------------------------------------------------
FAntigravityActionResult FAntigravityPerformanceActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	...
	if (ToolName == TEXT("set_cvar"))
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("PerformanceActions: CVar %s: %s -> %s"), *CVarName, *OldValue, *CVarValue);
	...
	if (ToolName == TEXT("execute_console_command"))
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("PerformanceActions: Executed console command: %s (result=%d)"), *Command, (int)bExecuted);
	...
	if (ToolName == TEXT("set_scalability_settings"))
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("PerformanceActions: Scalability levels updated (save=%d)"), (int)bSave);
	...
	if (ToolName == TEXT("set_renderer_setting"))
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("PerformanceActions: URendererSettings.%s = %s (saved to DefaultEngine.ini)"),
			*PropName, *Value);
