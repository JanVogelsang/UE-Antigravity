...
bool FAntigravityNiagaraActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
	...
	{
		OutErrors.Add(TEXT("Missing required field: system_path or asset_path"));
		return false;
	}
	return true;
}
...
FAntigravityActionResult FAntigravityNiagaraActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	...
	if (ToolName == TEXT("create_niagara_system"))          return ExecuteCreateSystem(Params, Result);
	if (ToolName == TEXT("add_niagara_emitter"))             return ExecuteAddEmitter(Params, Result);
	if (ToolName == TEXT("add_niagara_module"))              return ExecuteAddModule(Params, Result);
	if (ToolName == TEXT("set_niagara_module_pin"))          return ExecuteSetModulePin(Params, Result);
	if (ToolName == TEXT("compile_niagara_system"))          return ExecuteCompileSystem(Params, Result);
	if (ToolName == TEXT("capture_niagara_system_isolated")) return ExecuteCaptureIsolated(Params, Result);
