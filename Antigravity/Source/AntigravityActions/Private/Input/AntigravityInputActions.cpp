...
bool FAntigravityInputActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
	FString ToolName;
	...
	{
		if (!Params->HasField(TEXT("mapping_context_path")) || !Params->HasField(TEXT("action_path")) || !Params->HasField(TEXT("key")))
		{
			OutErrors.Add(TEXT("Missing required field(s) for add_input_mapping: mapping_context_path, action_path, key"));
			return false;
		}
	}
...
static FKey ParseKeyName(const FString& KeyName)
{
	...
	if (!Key.IsValid())
	{
		UE_LOG(LogAntigravity, Warning, TEXT("InputActions: Key '%s' not recognized, using as-is"), *KeyName);
	}
	return Key;
}

FAntigravityActionResult FAntigravityInputActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	FScopedTransaction Transaction(FText::FromString(TEXT("Antigravity Enhanced Input Action")));
...
// create_input_action
// ============================================================
FAntigravityActionResult FAntigravityInputActions::ExecuteCreateInputAction(const TSharedRef<FJsonObject>& Params)
{
	FAntigravityActionResult Result;
	Result.bSuccess = false;
	...
	UE_LOG(LogAntigravity, Log, TEXT("InputActions: Created IA '%s' at %s"), *AssetName, *AssetPath);
	return Result;
}
...
// ============================================================
FAntigravityActionResult FAntigravityInputActions::ExecuteCreateInputMappingContext(const TSharedRef<FJsonObject>& Params)
{
	FAntigravityActionResult Result;
	Result.bSuccess = false;
	...
	UE_LOG(LogAntigravity, Log, TEXT("InputActions: Created IMC '%s' at %s"), *AssetName, *AssetPath);
	return Result;
}
...
// add_input_mapping
// ============================================================
FAntigravityActionResult FAntigravityInputActions::ExecuteAddInputMapping(const TSharedRef<FJsonObject>& Params)
{
	FAntigravityActionResult Result;
	Result.bSuccess = false;
	...
	if (Params->TryGetArrayField(TEXT("modifiers"), ModifiersArray))
	{
		for (const TSharedPtr<FJsonValue>& ModVal : *ModifiersArray)
		{
			...
			{
				UE_LOG(LogAntigravity, Warning, TEXT("InputActions: Unknown modifier '%s', skipping"), *ModName);
			}
		}
	}
	...
	if (Params->TryGetArrayField(TEXT("triggers"), TriggersArray))
	{
		for (const TSharedPtr<FJsonValue>& TrigVal : *TriggersArray)
		{
			...
			{
				UE_LOG(LogAntigravity, Warning, TEXT("InputActions: Unknown trigger '%s', skipping"), *TrigName);
			}
			// "default" and empty strings are silently skipped â€” we add a default below
		}
	}
	...
	{
		Mapping.Triggers.Add(NewObject<UInputTriggerPressed>());
		UE_LOG(LogAntigravity, Log, TEXT("InputActions: No triggers specified â€” defaulting to Pressed trigger."));
	}
	...
	Result.bSuccess = true;
	Result.ResultMessage = FString::Printf(
		TEXT("Added mapping: %s â†’ %s (Key=%s, Modifiers=[%s], Triggers=[%s])"),
		*IA->GetName(), *IMC->GetName(), *KeyName, *ModsDesc, *TrigsDesc);
	Result.ModifiedPaths.Add(IMCPath);

	UE_LOG(LogAntigravity, Log, TEXT("InputActions: Added mapping %s â†’ %s (Key=%s)"),
