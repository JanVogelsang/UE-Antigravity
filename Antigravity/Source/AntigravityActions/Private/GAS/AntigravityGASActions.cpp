...
#define LOCTEXT_NAMESPACE "AntigravityGASActions"

// ============================================================================
// Helper: Set a protected UPROPERTY on a CDO via reflection
// ============================================================================

namespace AntigravityGASReflection
{
...
FAntigravityGASActions::FAntigravityGASActions() {}
FAntigravityGASActions::~FAntigravityGASActions() {}
...
FAntigravityActionResult FAntigravityGASActions::ExecuteRegisterTags(
	const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	...
	for (const TSharedPtr<FJsonValue>& TagVal : *TagsArray)
	{
		...
		TagsEditor.AddNewGameplayTagToINI(TagName, Comment.IsEmpty() ? TEXT("Added by Antigravity") : Comment);
...
FAntigravityActionResult FAntigravityGASActions::ExecuteCreateAttributeSet(
	const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString ClassName;
	...
	// Build header
	FString Header;
	Header += TEXT("// Generated by Antigravity â€” Gameplay Ability System AttributeSet\n\n");
...
FAntigravityActionResult FAntigravityGASActions::ExecuteSetupASC(
	const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath;
...
FAntigravityActionResult FAntigravityGASActions::ExecuteCreateEffect(
	const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath;
	if (!Params->TryGetStringField(TEXT("asset_path"), AssetPath))
		...
		for (const TSharedPtr<FJsonValue>& TagVal : *GrantTagsArray)
		{
			...
			if (TagVal->TryGetString(TagStr))
			{
				...
				if (Tag.IsValid())
				{
					...
					AntigravityGASReflection::AddTagToContainer(GECDO, TEXT("InheritableOwnedTagsContainer"), Tag);
	...
	if (Params->TryGetArrayField(TEXT("asset_tags"), AssetTagsArray))
	{
		for (const TSharedPtr<FJsonValue>& TagVal : *AssetTagsArray)
		{
			...
			if (TagVal->TryGetString(TagStr))
			{
				...
				if (Tag.IsValid())
				{
					AntigravityGASReflection::AddTagToContainer(GECDO, TEXT("InheritableGameplayEffectTags"), Tag);
...
FAntigravityActionResult FAntigravityGASActions::ExecuteCreateAbility(
	const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath;
	if (!Params->TryGetStringField(TEXT("asset_path"), AssetPath))
	...
	AntigravityGASReflection::SetEnumProperty(GACDO, TEXT("InstancingPolicy"), InstancingPolicy);
	...
	AntigravityGASReflection::SetEnumProperty(GACDO, TEXT("NetExecutionPolicy"), NetPolicy);
	...
	if (Params->TryGetArrayField(TEXT("ability_tags"), AbilityTagsArray))
	{
		for (const TSharedPtr<FJsonValue>& TagVal : *AbilityTagsArray)
		{
			...
			if (TagVal->TryGetString(TagStr))
			{
				...
				if (Tag.IsValid())
				{
					AntigravityGASReflection::AddTagToContainer(GACDO, TEXT("AbilityTags"), Tag);
	...
	if (Params->TryGetStringField(TEXT("cooldown_effect"), CooldownGEPath))
	{
		...
		{
			AntigravityGASReflection::SetClassProperty(GACDO, TEXT("CooldownGameplayEffectClass"), CooldownBP->GeneratedClass);
	...
	if (Params->TryGetStringField(TEXT("cost_effect"), CostGEPath))
	{
		...
		{
			AntigravityGASReflection::SetClassProperty(GACDO, TEXT("CostGameplayEffectClass"), CostBP->GeneratedClass);
