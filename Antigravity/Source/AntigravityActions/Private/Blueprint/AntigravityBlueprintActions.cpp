...
TMap<FString, int32> FAntigravityBlueprintActions::AssetModificationCounts;
...
bool FAntigravityBlueprintActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
	FString AssetPath;
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	FString ToolName;
	Params->TryGetStringField(TEXT("_tool_name"), ToolName);
	...
	if (!bIsReadOnly)
	{
		Transaction.Emplace(FText::FromString(TEXT("Antigravity Blueprint Action")));
	}
...
bool FAntigravityBlueprintActions::CompileAndReport(UBlueprint* Blueprint, FAntigravityActionResult& Result, bool bSkipGC)
{
	check(Blueprint);
...
UEdGraph* FAntigravityBlueprintActions::FindOrCreateEventGraph(UBlueprint* Blueprint, const FString& GraphName)
{
	check(Blueprint);
...
USCS_Node* FAntigravityBlueprintActions::FindSCSNodeByName(UBlueprint* Blueprint, const FString& NodeName)
{
...
FString FAntigravityBlueprintActions::ResolveT3DPlaceholders(const FString& T3DText)
{
...
bool FAntigravityBlueprintActions::DetectInfiniteLoopRisk(UBlueprint* Blueprint, TArray<FString>& OutWarnings) const
{
...
FString FAntigravityBlueprintActions::BuildBlueprintInfoJson(UBlueprint* Blueprint, const TArray<FString>* NodeNamesFilter, bool bExcludeVisualLayout, const FString& QueryMode)
{
...
bool FAntigravityBlueprintActions::ResolveEventMapping(const FString& EventName, FName& OutFunctionName, UClass*& OutOwnerClass)
{
	// Actor events
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteCreateBlueprint(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	...
	UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Created Blueprint '%s' with parent '%s'"), *AssetPath, *ParentClassName);
		...
		for (const TSharedPtr<FJsonValue>& CompValue : *ComponentsArray)
		{
			...
			if (CompClass && NewBlueprint->SimpleConstructionScript)
			{
				...
				if (NewNode)
				{
					...
					UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Added inline component '%s' (%s)"), *CompName, *CompClassName);
				}
			}
			else
			{
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteAddComponent(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath    = Params->GetStringField(TEXT("asset_path"));
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteAddVariable(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteAddFunction(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath    = Params->GetStringField(TEXT("asset_path"));
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteAddEventHandler(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath  = Params->GetStringField(TEXT("asset_path"));
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteCompileBlueprint(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteSetDefaults(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	for (const auto& Pair : (*DefaultsObj)->Values)
	{
		...
		if (Pair.Value->TryGetString(ValueStr))
		{
			...
			UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Set default '%s' = '%s' on '%s'"), *FullKey, *ValueStr, *TargetObject->GetClass()->GetName());
		}
		else
		{
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteSetComponentProperties(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath     = Params->GetStringField(TEXT("asset_path"));
	...
	if (Params->TryGetStringField(TEXT("static_mesh"), StaticMeshPath) && !StaticMeshPath.IsEmpty())
	{
		...
		else
		{
			...
			if (Mesh)
			{
				...
				UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Set StaticMesh '%s' on component '%s'"), *StaticMeshPath, *ComponentName);
			}
			else
			{
	...
	if (Params->TryGetStringField(TEXT("skeletal_mesh"), SkeletalMeshPath) && !SkeletalMeshPath.IsEmpty())
	{
		...
		else
		{
			...
			if (Mesh)
			{
				...
				UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Set SkeletalMesh '%s' on component '%s'"), *SkeletalMeshPath, *ComponentName);
			}
			else
			{
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteInjectNodesT3D(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString T3DText;
	...
	UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Injecting T3D nodes into graph '%s' of '%s'"), *GraphName, *AssetPath);
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteGetBlueprintInfo(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
...
bool FAntigravityBlueprintActions::ConnectPinsHelper(
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteConnectPins(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath;
	FString GraphName   = TEXT("EventGraph");
	FString SourceNode;
...
FString FAntigravityBlueprintActions::BuildPinAuditReport(const TSet<UEdGraphNode*>& Nodes)
{
...
FString FAntigravityBlueprintActions::BuildCompactConnectionReport(const TSet<UEdGraphNode*>& Nodes, const FString& GraphName)
{
...
void FAntigravityBlueprintActions::ResolvePinType(const FString& TypeName, FEdGraphPinType& OutPinType)
{
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				UE_LOG(LogAntigravity, Warning, TEXT("BlueprintActions: Unresolved pin type '%s' — using wildcard. Check the type name spelling."), *TypeName);
			}
		}
		}
	}
	...
	FAntigravityActionResult FAntigravityBlueprintActions::ExecuteAddEnhancedInputNode(
		const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Added Enhanced Input node '%s' for IA '%s' in '%s'"),
	...
	FAntigravityActionResult FAntigravityBlueprintActions::ExecuteVerifyConnections(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
	{
		FString AssetPath;
		if (!Params->TryGetStringField(TEXT("asset_path"), AssetPath))
		{
		...
		UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: VerifyConnections completed for '%s' — %d issues, %d repairs, %d unresolved"),
...
void FAntigravityBlueprintActions::AutoLayoutNodes(
	...
	int32 LayerSpacingX, int32 NodeSpacingY)
{
	...
	UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Auto-layout applied to %d nodes across %d layers"),
...
bool FAntigravityBlueprintActions::PreFlightValidateT3D(
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteSetNodePinDefault(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString NodeName;
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteDeleteNodes(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString GraphName = TEXT("EventGraph");
	Params->TryGetStringField(TEXT("graph_name"), GraphName);
	...
	for (UEdGraphNode* Node : NodesToRemove)
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Deleted node '%s' (%s) from graph '%s' of '%s'"),
...
FAntigravityActionResult FAntigravityBlueprintActions::ExecuteBatchOperations(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	UE_LOG(LogAntigravity, Log, TEXT("BlueprintActions: Executing batch of %d operations on '%s'"), OperationsArray->Num(), *AssetPath);
	...
	for (const TSharedPtr<FJsonValue>& OpVal : *OperationsArray)
	{
		...
		// Execute
		FAntigravityActionResult SubResult = ExecuteAction(SubParams);
