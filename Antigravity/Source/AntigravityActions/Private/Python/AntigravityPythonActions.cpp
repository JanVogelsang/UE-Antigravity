...
#define LOCTEXT_NAMESPACE "AntigravityPythonActions"

// ============================================================================
// Lifecycle
// ============================================================================

FAntigravityPythonActions::FAntigravityPythonActions()
{
	InitializeDenylist();
}
...
FAntigravityActionResult FAntigravityPythonActions::ExecutePythonScript(
	...
	FAntigravityActionResult& Result)
{
	...
	FString ScriptPath = FPaths::Combine(ScriptsDir, FString::Printf(TEXT("antigravity_script_%s.py"), *ScriptId));
	FString OutputPath = FPaths::Combine(ScriptsDir, FString::Printf(TEXT("antigravity_output_%s.txt"), *ScriptId));
	...
	FString WrappedScript = FString::Printf(TEXT(
		...
		"import traceback\n"
		"\n"
		"_antigravity_output_path = r'%s'\n"
		"_antigravity_stdout = io.StringIO()\n"
		"_antigravity_stderr = io.StringIO()\n"
		"_antigravity_old_stdout = sys.stdout\n"
		"_antigravity_old_stderr = sys.stderr\n"
		"sys.stdout = _antigravity_stdout\n"
		"sys.stderr = _antigravity_stderr\n"
		"\n"
		"try:\n"
		"    # === USER SCRIPT BEGIN ===\n"
		"%s\n"
		"    # === USER SCRIPT END ===\n"
		"except Exception as _antigravity_e:\n"
		"    print(f'ERROR: {type(_antigravity_e).__name__}: {_antigravity_e}', file=sys.stderr)\n"
		"    traceback.print_exc(file=sys.stderr)\n"
		"finally:\n"
		"    sys.stdout = _antigravity_old_stdout\n"
		"    sys.stderr = _antigravity_old_stderr\n"
		"    _out = _antigravity_stdout.getvalue()\n"
		"    _err = _antigravity_stderr.getvalue()\n"
		"    _combined = ''\n"
		"    if _out:\n"
		"        _combined += _out\n"
		"    if _err:\n"
		...
		"    with open(_antigravity_output_path, 'w', encoding='utf-8') as _f:\n"
		"        _f.write(_combined)\n"
...
#else
	Result.Errors.Add(TEXT("This build of Antigravity was compiled without Python support (WITH_PYTHON=0)."));
	IFileManager::Get().Delete(*ScriptPath);
	return Result;
#endif
	...
	UE_LOG(LogAntigravity, Log, TEXT("PythonActions: Script completed in %.1fs (success: %s)"),
...
FString FAntigravityPythonActions::IndentScript(const FString& Script)
{
...
bool FAntigravityPythonActions::IsPythonPluginAvailable() const
{
#if WITH_PYTHON
...
FString FAntigravityPythonActions::GetScriptsDir() const
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("Antigravity"), TEXT("Scripts"));
}

void FAntigravityPythonActions::InitializeDenylist()
{
...
bool FAntigravityPythonActions::ValidatePythonScript(const FString& Script, TArray<FString>& OutViolations) const
{
	bool bValid = true;
