// Copyright 2026 Antigravity. All Rights Reserved.

#pragma once

#include "CoreMinimal.h"
#include "AntigravityInterfaces.h"

/**
 * FAntigravityBehaviorTreeActions
 *
 * Gameplay AI tools for creating and configuring Behavior Trees:
 *   - create_blackboard: Create Blackboard assets with typed keys
 *   - create_behavior_tree: Create Behavior Tree assets and assign Blackboards
 *   - inject_bt_nodes: Add Selectors, Sequences, Decorators, Services programmatically
 *   - configure_navmesh: Spawn NavMeshBoundsVolumes and trigger NavMesh builds
 *
 * Behavior Trees are Unreal's primary AI decision-making system. Combined with
 * the Blackboard (shared memory), they let the AI create NPC behavior patterns
 * like patrol â†’ investigate â†’ attack â†’ flee.
 */
class ANTIGRAVITYACTIONS_API FAntigravityBehaviorTreeActions : public IAntigravityActionExecutor
{
public:
	FAntigravityBehaviorTreeActions();
	virtual ~FAntigravityBehaviorTreeActions();

	virtual FName GetActionName() const override;
	virtual FText GetDisplayName() const override;
	virtual EAntigravityActionCategory GetCategory() const override;
	virtual EAntigravityRiskLevel GetDefaultRiskLevel() const override;
	virtual FAntigravityActionPlan PreviewAction(const TSharedRef<FJsonObject>& Params) override;
	virtual FAntigravityActionResult ExecuteAction(const TSharedRef<FJsonObject>& Params) override;
	virtual bool CanUndo() const override;
	virtual bool UndoAction() override;
	virtual TArray<FString> GetSupportedToolNames() const override;
	virtual bool ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const override;

private:
	/** Create a Blackboard asset with typed keys (Bool, Int, Float, String, Vector, Object, Enum, Class). */
	FAntigravityActionResult ExecuteCreateBlackboard(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result);

	/** Create a Behavior Tree asset and optionally assign a Blackboard. */
	FAntigravityActionResult ExecuteCreateBehaviorTree(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result);

	/** Inject nodes into an existing BT (Selectors, Sequences, Tasks, Decorators, Services). */
	FAntigravityActionResult ExecuteInjectBTNodes(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result);

	/** Spawn a NavMeshBoundsVolume, scale to level, and trigger NavMesh rebuild. */
	FAntigravityActionResult ExecuteConfigureNavMesh(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result);
};
