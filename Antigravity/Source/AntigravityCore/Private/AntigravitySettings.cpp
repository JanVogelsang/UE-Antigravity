...
{
	// Provider selection â€” defaults to Anthropic (backward compat)
	ActiveProvider = EAntigravityProvider::Anthropic;

	// --- Anthropic defaults ---
	ApiEndpoint = DefaultApiEndpoint;
	ClaudeModel = EAntigravityClaudeModel::Sonnet_4_6;
	ContextWindow = EAntigravityContextWindow::Standard_200K;
	...
	OpenAiReasoningEffort = EAntigravityReasoningEffort::Medium;
	...
	GeminiReasoningEffort = EAntigravityReasoningEffort::Disabled;  // Disabled by default. Prevents sending unknown fields to non-thinking models.
...
bool UAntigravityDeveloperSettings::IsApiKeySet() const
{
	return !ApiKey.IsEmpty() && ApiKey.Len() > 10;
}

FString UAntigravityDeveloperSettings::GetEffectiveEndpoint() const
{
	switch (ActiveProvider)
	{
	case EAntigravityProvider::Anthropic:
		return ApiEndpoint.IsEmpty() ? DefaultApiEndpoint : ApiEndpoint;
	case EAntigravityProvider::OpenAI:
		// Official OpenAI API only â€” for Azure, use the Azure provider.
	...
	case EAntigravityProvider::Azure:
	...
	case EAntigravityProvider::Ollama:
	{
		...
		//
		// Users commonly set "http://localhost:11434" without /v1 â†’ 404.
	...
	case EAntigravityProvider::LMStudio:
	{
	...
	case EAntigravityProvider::Custom:
...
FString UAntigravityDeveloperSettings::GetActiveApiKey() const
{
	switch (ActiveProvider)
	{
	case EAntigravityProvider::Anthropic:  return ApiKey;
	case EAntigravityProvider::OpenAI:     return OpenAiApiKey;
	case EAntigravityProvider::Azure:      return AzureApiKey;   // 'api-key' header, not 'Authorization: Bearer'
	case EAntigravityProvider::Google:     return GeminiApiKey;
	case EAntigravityProvider::DeepSeek:   return DeepSeekApiKey;
	case EAntigravityProvider::Mistral:    return MistralApiKey;
	case EAntigravityProvider::xAI:        return xAIApiKey;
	case EAntigravityProvider::OpenRouter: return OpenRouterApiKey;
	case EAntigravityProvider::Ollama:     return TEXT("");   // no auth for local
	case EAntigravityProvider::LMStudio:   return TEXT("");   // no auth for local
	case EAntigravityProvider::Custom:     return CustomApiKey;
	default: return ApiKey;
	}
}

FString UAntigravityDeveloperSettings::GetEffectiveModel() const
{
	switch (ActiveProvider)
	{
	case EAntigravityProvider::Anthropic:
	{
		if (ClaudeModel == EAntigravityClaudeModel::Custom)
			return CustomModelId.IsEmpty() ? ModelEnumToApiString(EAntigravityClaudeModel::Sonnet_4_6) : CustomModelId;
	...
	case EAntigravityProvider::GitHubCopilot:
		return CopilotModelId.IsEmpty() ? TEXT("gpt-4") : CopilotModelId;
	default:
		return ModelEnumToApiString(EAntigravityClaudeModel::Sonnet_4_6);
	}
}

bool UAntigravityDeveloperSettings::IsActiveProviderApiKeySet() const
{
	FString Key = GetActiveApiKey();
	// Local providers and Copilot don't need an API key configured here
	if (ActiveProvider == EAntigravityProvider::Ollama || 
		ActiveProvider == EAntigravityProvider::LMStudio || 
		ActiveProvider == EAntigravityProvider::GitHubCopilot)
	{
		return true;
	}
	// Azure: also requires AzureBaseUrl and AzureDeploymentName to be usable
	if (ActiveProvider == EAntigravityProvider::Azure)
		return !Key.IsEmpty() && Key.Len() > 10 && !AzureBaseUrl.IsEmpty() && !AzureDeploymentName.IsEmpty();
	return !Key.IsEmpty() && Key.Len() > 10;
}

FString UAntigravityDeveloperSettings::GetModelDisplayName() const
{
	...
	case EAntigravityProvider::Anthropic:
	{
		switch (ClaudeModel)
		{
		case EAntigravityClaudeModel::Sonnet_4_6:	return TEXT("Claude Sonnet 4.6");
		case EAntigravityClaudeModel::Sonnet_4_5:	return TEXT("Claude Sonnet 4.5");
		case EAntigravityClaudeModel::Opus_4_6:	return TEXT("Claude Opus 4.6");
		case EAntigravityClaudeModel::Opus_4_5:	return TEXT("Claude Opus 4.5");
		case EAntigravityClaudeModel::Haiku_4:	return TEXT("Claude Haiku 4");
		case EAntigravityClaudeModel::Custom:		return FString::Printf(TEXT("Custom (%s)"), *CustomModelId);
...
FString UAntigravityDeveloperSettings::ModelEnumToApiString(EAntigravityClaudeModel Model)
{
	switch (Model)
	{
	case EAntigravityClaudeModel::Sonnet_4_6:	return TEXT("claude-sonnet-4-6");
	case EAntigravityClaudeModel::Sonnet_4_5:	return TEXT("claude-sonnet-4-5-20250929");
	case EAntigravityClaudeModel::Opus_4_6:	return TEXT("claude-opus-4-6");
	case EAntigravityClaudeModel::Opus_4_5:	return TEXT("claude-opus-4-5");
	case EAntigravityClaudeModel::Haiku_4:	return TEXT("claude-haiku-4");
...
bool UAntigravityDeveloperSettings::IsToolCategoryAllowed(EAntigravityActionCategory Category) const
{
	switch (SecurityMode)
	{
	case EAntigravitySecurityMode::Sandbox:
		switch (Category)
		{
		case EAntigravityActionCategory::Cpp:
		case EAntigravityActionCategory::Build:
		case EAntigravityActionCategory::Settings:
		case EAntigravityActionCategory::SourceControl:
		case EAntigravityActionCategory::FileSystem:
			return false;
		default:
			return true;
		}
	case EAntigravitySecurityMode::Advanced:
		switch (Category)
		{
		case EAntigravityActionCategory::Build:
			return false;
	...
	case EAntigravitySecurityMode::Developer:
		return true;
	default:
		return false;
	}
...
{
	return FText::FromString(TEXT("Configure the Antigravity AI assistant plugin -- API keys, model selection, safety settings, context, and tool availability."));
}
...
void UAntigravityDeveloperSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	// ====================================================================
	if (PropName == GET_MEMBER_NAME_CHECKED(UAntigravityDeveloperSettings, ThinkingBudgetTokens))
	{
		...
		if (bEnableExtendedThinking && MaxResponseTokens <= ThinkingBudgetTokens)
		{
			...
			UE_LOG(LogAntigravity, Warning,
				TEXT("Antigravity: MaxResponseTokens auto-adjusted to %d (must be > ThinkingBudgetTokens %d)"),
				MaxResponseTokens, ThinkingBudgetTokens);
		}
	}
	else if (PropName == GET_MEMBER_NAME_CHECKED(UAntigravityDeveloperSettings, MaxResponseTokens))
	{
		if (bEnableExtendedThinking && MaxResponseTokens <= ThinkingBudgetTokens)
		{
			MaxResponseTokens = ThinkingBudgetTokens + 1024;
			UE_LOG(LogAntigravity, Warning,
				TEXT("Antigravity: MaxResponseTokens clamped to %d (must be > ThinkingBudgetTokens %d)"),
				MaxResponseTokens, ThinkingBudgetTokens);
		}
	}
	else if (PropName == GET_MEMBER_NAME_CHECKED(UAntigravityDeveloperSettings, bEnableExtendedThinking))
	{
...
TArray<FString> UAntigravityDeveloperSettings::GetOpenAIModelOptions() const
{
...
TArray<FString> UAntigravityDeveloperSettings::GetGeminiModelOptions() const
{
...
TArray<FString> UAntigravityDeveloperSettings::GetDeepSeekModelOptions() const
{
...
TArray<FString> UAntigravityDeveloperSettings::GetMistralModelOptions() const
{
...
TArray<FString> UAntigravityDeveloperSettings::GetxAIModelOptions() const
{
...
TArray<FString> UAntigravityDeveloperSettings::GetCopilotModelOptions() const
{
