 ...
 */
struct ANTIGRAVITYENGINE_API FAntigravityCheckpoint
{
	...
	FAntigravityCheckpoint() = default;
};

/**
 * Diff information between two checkpoints.
 */
struct ANTIGRAVITYENGINE_API FAntigravityCheckpointDiff
{
...
/**
 * Git-based checkpoint system for Antigravity sessions.
 *
 * Creates a shadow git repository in Saved/Antigravity/Checkpoints/[SessionId]/
 * that mirrors the project's tracked file state. At each tool execution batch,
 * a git commit is created. This allows:
 *
 *   1. checkpointSave()    â€” commit current state to shadow repo
 *   2. checkpointRestore() â€” restore files to a previous checkpoint
 *   3. checkpointDiff()    â€” show what changed between any two checkpoints
 *
 ...
 *   - Restore does NOT rewind conversation history (that's handled by SAntigravityMainPanel)
 *
 * REQUIREMENTS:
 ...
 *   [ProjectRoot]/Saved/Antigravity/Checkpoints/[SessionId]/
 *
 * FILES TRACKED:
 ...
 */
class ANTIGRAVITYENGINE_API FAntigravityCheckpointManager
{
public:
	FAntigravityCheckpointManager();
	~FAntigravityCheckpointManager();

	// Non-copyable
	FAntigravityCheckpointManager(const FAntigravityCheckpointManager&) = delete;
	FAntigravityCheckpointManager& operator=(const FAntigravityCheckpointManager&) = delete;
	 ...
	 * Does NOT touch the Antigravity conversation history.
	 *
	 ...
	 */
	FAntigravityCheckpointDiff GetDiff(
	...
	TArray<FAntigravityCheckpoint> GetAllCheckpoints() const;

	/** Get the most recent checkpoint */
	const FAntigravityCheckpoint* GetLatestCheckpoint() const;

	/** Get a checkpoint by commit hash */
	const FAntigravityCheckpoint* GetCheckpointByHash(const FString& CommitHash) const;
...
private:
	...
	TArray<FAntigravityCheckpoint> Checkpoints;
