 ...
 */
struct ANTIGRAVITYENGINE_API FAntigravityToolRepetitionCheck
{
 ...
 *
 * Integration: call Check() before ExecuteToolCall() in SAntigravityMainPanel.
 * If bAllowExecution == false, pause the agentic loop and present WarningMessage.
 */
class ANTIGRAVITYENGINE_API FAntigravityToolRepetitionDetector
{
public:
	/** Default: block after 3 consecutive identical calls (same as Roo Code default) */
	static constexpr int32 DefaultMaxRepetitions = 3;

	explicit FAntigravityToolRepetitionDetector(int32 InMaxRepetitions = DefaultMaxRepetitions);
	 ...
	 * @return          Check result â€” bAllowExecution=false means block + show WarningMessage
	 */
	FAntigravityToolRepetitionCheck Check(const FAntigravityToolCall& ToolCall);
...
private:
	...
	FString SerializeToolCall(const FAntigravityToolCall& ToolCall) const;
