...
class FAntigravityFileContextTracker;
 ...
 *     1. SYSTEM PROMPT   â€” built once at session start, static codebase overview
 *     2. ENVIRONMENT DETAILS â€” rebuilt on every API call, captures current editor state
 *
 *   Antigravity currently puts EVERYTHING into BuildSystemPrompt() which grows stale
 ...
 */
class ANTIGRAVITYENGINE_API FAntigravityEnvironmentDetails
{
public:
	FAntigravityEnvironmentDetails();
	~FAntigravityEnvironmentDetails();
	 ...
	 */
	void SetFileContextTracker(FAntigravityFileContextTracker* InTracker);

	/**
	 * Attach an IgnoreController to filter paths from environment output.
	 * Does NOT take ownership.
	 */
	void SetIgnoreController(FAntigravityIgnoreController* InIgnoreController);
	...
	FString GetTodoSummarySection(const TArray<FAntigravityTodoItem>& Todos) const;
...
private:
	FAntigravityFileContextTracker* FileContextTracker = nullptr;
