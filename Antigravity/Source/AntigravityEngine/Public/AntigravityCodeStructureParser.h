 ...
 */
struct ANTIGRAVITYENGINE_API FAntigravityCodeDeclaration
{
	/** Line number (1-based) */
	int32 LineNumber = 0;
 ...
 */
struct ANTIGRAVITYENGINE_API FAntigravityFileStructure
{
	/** Project-relative file path */
	FString RelativePath;

	/** All parsed declarations in order */
	TArray<FAntigravityCodeDeclaration> Declarations;
 ...
 *
 * This is Antigravity's equivalent of Roo Code's tree-sitter service
 ...
 *
 * USAGE (by AntigravityContextCondenser after condensation):
 *   TArray<FString> FilesRead = FileContextTracker->GetTrackedFilePaths();
 *   for (const FString& File : FilesRead) {
 *       FAntigravityFileStructure Structure = Parser.ParseFile(File);
 ...
 */
class ANTIGRAVITYENGINE_API FAntigravityCodeStructureParser
{
public:
	 ...
	 */
	FAntigravityFileStructure ParseFile(const FString& AbsolutePath, const FString& RelativePath) const;
...
private:
	...
	FAntigravityFileStructure ParseCppFile(const FString& Content, const FString& RelativePath) const;
