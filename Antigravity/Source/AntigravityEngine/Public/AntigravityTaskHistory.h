 ...
 * Plain C++ struct â€” no UHT/GENERATED_BODY needed since this is
 * a persistence-only record, not exposed to Blueprint.
 */
struct ANTIGRAVITYENGINE_API FAntigravityTaskHistoryItem
{
	...
	FAntigravityTokenUsage TotalTokenUsage;
 ...
 * Provides a browsable history of all past Antigravity sessions.
 *
 * STORAGE:
 *   Saved/Antigravity/TaskHistory/task_history.json
 ...
 */
class ANTIGRAVITYENGINE_API FAntigravityTaskHistory
{
public:
	FAntigravityTaskHistory();
	~FAntigravityTaskHistory();

	/** Initialize â€” creates or loads the history file */
	bool Initialize();

	/** Record a new or updated task entry */
	void RecordTask(const FAntigravityTaskHistoryItem& Item);

	/** Update an existing task's metadata */
	void UpdateTask(const FString& TabId, const FAntigravityTaskHistoryItem& UpdatedItem);
	...
	TArray<FAntigravityTaskHistoryItem> GetHistory() const;

	/** Get a specific task by ID */
	const FAntigravityTaskHistoryItem* GetTask(const FString& TabId) const;
...
private:
	...
	TArray<FAntigravityTaskHistoryItem> HistoryItems;
