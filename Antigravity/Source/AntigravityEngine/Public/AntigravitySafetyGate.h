 ...
 * IgnoreController integration for .antigravityignore enforcement.
 *
 * Protected paths differ from AllowedWritePaths:
 *   - AllowedWritePaths: whitelist â€” only these paths are writable
 ...
 *
 * Any path in ProtectedPaths returns EAntigravityRiskLevel::Critical for write
 * operations regardless of SecurityMode.
 */
class ANTIGRAVITYENGINE_API FAntigravitySafetyGate : public IAntigravitySafetyGate
{
public:
	FAntigravitySafetyGate();
	virtual ~FAntigravitySafetyGate();

	// IAntigravitySafetyGate interface
	virtual EAntigravityRiskLevel EvaluateRisk(const FAntigravityAction& Action) const override;
	virtual bool IsActionAllowed(const FAntigravityAction& Action, FString& OutReason) const override;
	 ...
	 * Protected paths produce EAntigravityRiskLevel::Critical for write operations.
	...
	/**
	 * Attach an AntigravityIgnoreController instance.
	 * When set, IsPathIgnoredByFilter() will use it to block access.
	 * Does NOT take ownership â€” caller owns the controller.
	 */
	void SetIgnoreController(FAntigravityIgnoreController* InController);

	/**
	 * Check if a path is blocked by the .antigravityignore file.
...
private:
	 ...
	 * UAntigravityDeveloperSettings. Called at construction so user-configured
