...
/**
 * Append-only execution journal recording every tool action Antigravity performs.
 ...
 * Writes to: Saved/Antigravity/ExecutionLog_YYYYMMDD.json
 */
class ANTIGRAVITYENGINE_API FAntigravityExecutionJournal
{
public:
	FAntigravityExecutionJournal();
	~FAntigravityExecutionJournal();

	/** Record an action execution to the journal */
	void RecordExecution(const FAntigravityActionExecutionRecord& Record);

	/** Get all records from the current session */
	const TArray<FAntigravityActionExecutionRecord>& GetSessionRecords() const { return SessionRecords; }
...
private:
	TArray<FAntigravityActionExecutionRecord> SessionRecords;
	bool bDirty;
	TSharedPtr<FJsonObject> RecordToJson(const FAntigravityActionExecutionRecord& Record) const;
};
