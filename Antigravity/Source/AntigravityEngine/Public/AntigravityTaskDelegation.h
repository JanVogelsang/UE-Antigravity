 ...
 */
enum class EAntigravitySubTaskStatus : uint8
{
 ...
 */
struct ANTIGRAVITYENGINE_API FAntigravitySubTask
{
	...
	EAntigravityAgentMode Mode = EAntigravityAgentMode::General;
	...
	/** Current status */
	EAntigravitySubTaskStatus Status = EAntigravitySubTaskStatus::Pending;
 ...
 */
DECLARE_DELEGATE_ThreeParams(FOnAntigravitySubTaskCompleted,
 ...
 *
 * USAGE IN SAntigravityMainPanel:
 ...
 *
 * TAB RELATIONSHIP:
 *   Parent tab â†’ spawns child tab â†’ child runs â†’ calls attempt_completion
 *   â†’ parent's OnSubTaskCompleted fires â†’ parent resumes with child's result
 *
 * Each tab maintains:
 ...
 */
class ANTIGRAVITYENGINE_API FAntigravityTaskDelegation
{
public:
	FAntigravityTaskDelegation();
	~FAntigravityTaskDelegation();
	...
	/** Get sub-task by ID */
	const FAntigravitySubTask* GetSubTask(const FString& SubTaskId) const;

	/** Get all active sub-tasks for a parent tab */
	TArray<FAntigravitySubTask*> GetActiveSubTasksForTab(const FString& ParentTabId);

	/** Get the sub-task associated with a child tab */
	const FAntigravitySubTask* GetSubTaskByChildTab(const FString& ChildTabId) const;
	...
	const TArray<FAntigravitySubTask>& GetAllSubTasks() const { return SubTasks; }

	// ---- Delegates ----

	/** Fired when a sub-task completes. Bind in SAntigravityMainPanel. */
	FOnAntigravitySubTaskCompleted OnSubTaskCompleted;

private:
	TArray<FAntigravitySubTask> SubTasks;
