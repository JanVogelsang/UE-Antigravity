...
#include "AntigravityCheckpointManager.h"
...
FAntigravityCheckpointManager::FAntigravityCheckpointManager()
{
}
...
bool FAntigravityCheckpointManager::Initialize(const FString& InSessionId, const FString& InProjectRoot)
{
	if (!IsGitAvailable())
	{
		UE_LOG(LogTemp, Warning, TEXT("AntigravityCheckpointManager: Git is not available. Checkpoint system disabled."));
		return false;
	}
	...
	// Shadow repo lives in Saved/Antigravity/Checkpoints/[SessionId]/
	...
	if (!bRepoExists)
	{
		...
		{
			UE_LOG(LogTemp, Error, TEXT("AntigravityCheckpointManager: Failed to init git repo at %s: %s"),
		...
		RunGitCommand(TEXT("commit -m \"Antigravity: initial commit\" --allow-empty"), Output, ErrOutput);
	...
	else
	{
		...
		UE_LOG(LogTemp, Log, TEXT("AntigravityCheckpointManager: Resuming existing checkpoint session. Initial hash: %s"), *InitialCommitHash);
	}

	bIsInitialized = true;
	UE_LOG(LogTemp, Log, TEXT("AntigravityCheckpointManager: Initialized shadow repo at %s"), *ShadowRepoPath);
	return true;
}
...
bool FAntigravityCheckpointManager::SaveCheckpoint(
	const FString& Description,
	int32 LoopIteration,
	FAntigravityCheckpoint& OutCheckpoint
)
{
	if (!bIsInitialized)
	{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("AntigravityCheckpointManager: Failed to sync files to shadow repo."));
		return false;
	}

	if (SyncedFiles.Num() == 0)
	{
		UE_LOG(LogTemp, Verbose, TEXT("AntigravityCheckpointManager: No files changed â€” skipping checkpoint."));
		return false;
	}
	...
	if (AddResult != 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("AntigravityCheckpointManager: git add failed (exit %d): %s"), AddResult, *ErrOutput);
		return false;
	}
	...
	if (DiffResult == 0)
	{
		UE_LOG(LogTemp, Verbose, TEXT("AntigravityCheckpointManager: No staged changes for checkpoint '%s' â€” skipping."), *Description);
		return false;
	}

	// Create commit
	const FString CommitMsg = FString::Printf(
		TEXT("Antigravity: %s (loop %d)"),
		*Description,
		LoopIteration
	);
	...
	if (CommitResult != 0)
	{
		// May fail if nothing to commit â€” not an error (check both Output and ErrOutput since
		...
		{
			UE_LOG(LogTemp, Verbose, TEXT("AntigravityCheckpointManager: Nothing to commit for checkpoint '%s'."), *Description);
			return false;
		}
		UE_LOG(LogTemp, Warning, TEXT("AntigravityCheckpointManager: Commit failed (exit %d): %s"), CommitResult, *ErrOutput);
		return false;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("AntigravityCheckpointManager: Saved checkpoint '%s' -> %s (%d files)"),
...
bool FAntigravityCheckpointManager::RestoreToCheckpoint(
...
FAntigravityCheckpointDiff FAntigravityCheckpointManager::GetDiff(
	const FString& FromHash,
	const FString& ToHash,
	const FString& DiffMode
)
{
	FAntigravityCheckpointDiff Result;
...
TArray<FAntigravityCheckpoint> FAntigravityCheckpointManager::GetAllCheckpoints() const
{
	return Checkpoints;
}

const FAntigravityCheckpoint* FAntigravityCheckpointManager::GetLatestCheckpoint() const
{
	return Checkpoints.Num() > 0 ? &Checkpoints.Last() : nullptr;
}

const FAntigravityCheckpoint* FAntigravityCheckpointManager::GetCheckpointByHash(const FString& CommitHash) const
{
	for (const FAntigravityCheckpoint& CP : Checkpoints)
	{
...
FString FAntigravityCheckpointManager::GetInitialCommitHash() const
{
...
int32 FAntigravityCheckpointManager::RunGitCommand(
	const FString& Args,
	FString& Output,
	FString& ErrOutput
) const
...
bool FAntigravityCheckpointManager::SyncFilesToShadowRepo(TArray<FString>& OutSyncedFiles)
{
...
bool FAntigravityCheckpointManager::SyncFilesFromShadowRepo(
	const FString& CommitHash,
	TArray<FString>& OutRestoredFiles
)
{
	...
	if (RunGitCommand(*FilesArg, Output, ErrOutput) != 0)
	{
		UE_LOG(LogTemp, Error, TEXT("AntigravityCheckpointManager: Failed to list files at %s: %s"), *CommitHash, *ErrOutput);
		return false;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("AntigravityCheckpointManager: Restored %d files from checkpoint %s"),
		OutRestoredFiles.Num(), *CommitHash.Left(8));

	return OutRestoredFiles.Num() > 0;
}

TArray<FString> FAntigravityCheckpointManager::GatherTrackedFiles() const
{
...
bool FAntigravityCheckpointManager::ShouldTrackFile(const FString& AbsolutePath) const
{
...
FString FAntigravityCheckpointManager::GetShadowRepoGitignore()
{
	return
		TEXT("# Antigravity shadow repo - binary files excluded\n")
		TEXT("*.uasset\n")
		TEXT("*.umap\n")
		TEXT("*.pak\n")
		TEXT("*.pdb\n")
...
bool FAntigravityCheckpointManager::ConfigureGitUser()
{
	FString Output, ErrOutput;
	RunGitCommand(TEXT("config user.name \"Antigravity\""), Output, ErrOutput);
	RunGitCommand(TEXT("config user.email \"antigravity@local\""), Output, ErrOutput);
	return true;
}

FString FAntigravityCheckpointManager::GetGitPath()
{
#if PLATFORM_WINDOWS
...
bool FAntigravityCheckpointManager::IsGitAvailable()
{
...
FString FAntigravityCheckpointManager::GetGitVersion()
{
	void* PipeRead = nullptr;
	void* PipeWrite = nullptr;
	FPlatformProcess::CreatePipe(PipeRead, PipeWrite);
