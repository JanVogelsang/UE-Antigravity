...
#include "AntigravityToolRepetitionDetector.h"
#include "Dom/JsonObject.h"
#include "Dom/JsonValue.h"
#include "Serialization/JsonSerializer.h"
#include "Serialization/JsonWriter.h"

FAntigravityToolRepetitionDetector::FAntigravityToolRepetitionDetector(int32 InMaxRepetitions)
...
FAntigravityToolRepetitionCheck FAntigravityToolRepetitionDetector::Check(const FAntigravityToolCall& ToolCall)
{
	const FString Serialized = SerializeToolCall(ToolCall);

	// New distinct tool call â€” reset counter entirely
	...
	if (bLimitReached)
	{
		// Reset everything â€” give Claude a fresh start
		Reset();
		LastSerializedCall = Serialized;
		ConsecutiveCount = 1;
		return { true, FString(), ConsecutiveCount };
	...
	if (ConsecutiveCount >= MaxRepetitions)
	{
		...
		const FString WarningMessage = FString::Printf(
			TEXT("Antigravity has detected that the AI is stuck in a repetition loop.\n\n"
				 ...
				 "  â€¢ Provide feedback to guide the AI (e.g., what error is occurring)\n"
				 "  â€¢ Cancel and start a new conversation with more context\n"
				 "  â€¢ Allow Antigravity to try once more\n\n"
				 "Tool: %s\nConsecutive calls: %d"),
			*ToolCall.ToolName,
			ConsecutiveCount,
			*ToolCall.ToolName,
...
void FAntigravityToolRepetitionDetector::Reset()
{
	ConsecutiveCount = 0;
...
FString FAntigravityToolRepetitionDetector::SerializeToolCall(const FAntigravityToolCall& ToolCall) const
{
