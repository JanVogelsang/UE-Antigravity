...
FAntigravityActionRouter::FAntigravityActionRouter() {}
FAntigravityActionRouter::~FAntigravityActionRouter() {}

void FAntigravityActionRouter::RegisterExecutor(TSharedRef<IAntigravityActionExecutor> Executor)
{
	for (const FString& ToolName : Executor->GetSupportedToolNames())
	{
		ExecutorMap.Add(ToolName, Executor);
		UE_LOG(LogAntigravity, Log, TEXT("ActionRouter: Registered executor for tool '%s'"), *ToolName);
	}
}

void FAntigravityActionRouter::ClearExecutors()
{
	ExecutorMap.Empty();
}
...
struct FAntigravityDiagnostics
{
...
FAntigravityActionResult FAntigravityActionRouter::RouteToolCall(const FAntigravityToolCall& ToolCall)
{
	...
	// paths, which freezes the editor. We catch this early and return a clean error.
	{
		...
		if (!ValidateAssetPathParam(ToolCall.InputParams, ValidationError))
		{
			...
			UE_LOG(LogAntigravity, Warning, TEXT("ActionRouter: Blocked tool '%s' â€” invalid asset_path: %s"),
	...
	if (!Executor->ValidateParams(ParamsWithToolName, ValidationErrors))
	{
		...
		UE_LOG(LogAntigravity, Warning, TEXT("ActionRouter: Blocked tool '%s' due to parameter validation failure"), *ToolCall.ToolName);
		return Result;
	}

	FAntigravityActionResult Result = Executor->ExecuteAction(ParamsWithToolName);

	if (!Result.bSuccess || Result.Errors.Num() > 0)
	{
		for (FString& ErrorMsg : Result.Errors)
		{
			FAntigravityDiagnostics::EnrichErrorString(ErrorMsg);
		}
	}
...
{
	const TSharedRef<IAntigravityActionExecutor>* Found = ExecutorMap.Find(ToolName);
	return Found ? TSharedPtr<IAntigravityActionExecutor>(*Found) : nullptr;
}
