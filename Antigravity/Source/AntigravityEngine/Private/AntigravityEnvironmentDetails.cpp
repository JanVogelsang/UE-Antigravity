...
#include "AntigravityEnvironmentDetails.h"
...
FAntigravityEnvironmentDetails::FAntigravityEnvironmentDetails()
{
}
...
void FAntigravityEnvironmentDetails::SetFileContextTracker(FAntigravityFileContextTracker* InTracker)
{
	FileContextTracker = InTracker;
}

void FAntigravityEnvironmentDetails::SetIgnoreController(FAntigravityIgnoreController* InIgnoreController)
{
	IgnoreController = InIgnoreController;
}

FString FAntigravityEnvironmentDetails::Build(
	float ContextUsagePercent,
	const TArray<FAntigravityTodoItem>& Todos,
...
) const
{
	...
	// Stale files section â€” most critical, goes first
...
FString FAntigravityEnvironmentDetails::GetOpenFilesSection() const
{
...
FString FAntigravityEnvironmentDetails::GetActiveLevelSection() const
{
...
FString FAntigravityEnvironmentDetails::GetSelectedActorsSection() const
{
...
FString FAntigravityEnvironmentDetails::GetCompileErrorsSection() const
{
...
FString FAntigravityEnvironmentDetails::GetContextWindowSection(float UsagePercent) const
{
	FString Status;
	FString Advice;
	...
	{
		Status = TEXT("âš ï¸ CRITICAL");
		Advice = TEXT(
			"\nâš ï¸ CRITICAL: Context window is almost full! You MUST:"
			...
			"\n  3. Do NOT start any new multi-step operations â€” there is not enough room."
			"\n  4. Keep responses extremely concise. Omit explanations."
		);
	}
	else if (UsagePercent >= 75.0f)
	{
		Status = TEXT("âš ï¸ High");
		Advice = TEXT(
...
FString FAntigravityEnvironmentDetails::GetTodoSummarySection(const TArray<FAntigravityTodoItem>& Todos) const
{
	if (Todos.Num() == 0)
	{
		return FString();
	...
	int32 Completed = 0;
	for (const FAntigravityTodoItem& Todo : Todos)
	{
		if (Todo.Status == EAntigravityTodoStatus::Completed) Completed++;
	}
	...
	for (const FAntigravityTodoItem& Todo : Todos)
	{
		...
		switch (Todo.Status)
		{
		case EAntigravityTodoStatus::Completed:  Marker = TEXT("[x]"); break;
		case EAntigravityTodoStatus::InProgress: Marker = TEXT("[-]"); break;
...
FString FAntigravityEnvironmentDetails::GetStaleFilesSection() const
{
	...
	// Build the warning â€” FileContextTracker clears stale flags when warning is built
	// We use const_cast here because BuildStaleContextWarning() clears state
	// (intentionally side-effectful, same as Roo Code design)
	return const_cast<FAntigravityFileContextTracker*>(FileContextTracker)->BuildStaleContextWarning();
}

FString FAntigravityEnvironmentDetails::GetSessionInfoSection(const FString& TabTitle, int32 LoopIteration) const
{
...
bool FAntigravityEnvironmentDetails::IsPathAllowed(const FString& Path) const
{
	if (!IgnoreController)
	{
		return true;
...
TArray<FString> FAntigravityEnvironmentDetails::GatherCompileErrors() const
{
	...
	// Note: FMessageLog doesn't expose a direct GetMessages() â€” we read from
...
TArray<FString> FAntigravityEnvironmentDetails::GatherOpenFiles() const
{
...
TArray<FString> FAntigravityEnvironmentDetails::GatherSelectedActors() const
{
...
FString FAntigravityEnvironmentDetails::GetActiveLevelName() const
{
#if WITH_EDITOR
	if (GEditor && GEditor->GetEditorWorldContext().World())
	{
