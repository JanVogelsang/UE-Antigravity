...
#include "AntigravityReferenceParser.h"
...
FAntigravityReferenceParser::FAntigravityReferenceParser()
{
}
...
void FAntigravityReferenceParser::SetIgnoreController(FAntigravityIgnoreController* InController)
{
...
FAntigravityParseReferencesResult FAntigravityReferenceParser::ParseAndResolve(const FString& InputText) const
{
	FAntigravityParseReferencesResult Result;
	...
	{
		FAntigravityResolvedReference Resolved;
...
TArray<FString> FAntigravityReferenceParser::ExtractReferences(const FString& Text) const
{
	TArray<FString> Refs;
...
FAntigravityResolvedReference FAntigravityReferenceParser::ResolveFileReference(const FString& RelativePath) const
{
	FAntigravityResolvedReference Result;
	Result.OriginalRef = RelativePath;
	Result.Type = EAntigravityReferenceType::File;

	// Check ignore controller
	if (IgnoreController && IgnoreController->IsPathIgnored(RelativePath))
	{
		Result.bSuccess = false;
		Result.ErrorMessage = FString::Printf(TEXT("File '%s' is excluded by .antigravityignore"), *RelativePath);
		return Result;
	}
...
FAntigravityResolvedReference FAntigravityReferenceParser::ResolveFolderReference(const FString& RelativePath) const
{
	FAntigravityResolvedReference Result;
	Result.OriginalRef = RelativePath;
	Result.Type = EAntigravityReferenceType::Folder;
...
FAntigravityResolvedReference FAntigravityReferenceParser::ResolveSpecialReference(const FString& Keyword) const
{
	FAntigravityResolvedReference Result;
	...
	{
		Result.Type = EAntigravityReferenceType::CompileErrors;
	...
	{
		Result.Type = EAntigravityReferenceType::Selection;
	...
	{
		Result.Type = EAntigravityReferenceType::Level;
	...
	{
		Result.Type = EAntigravityReferenceType::BuildLog;
	...
	{
		Result.Type = EAntigravityReferenceType::BlueprintAsset;
...
TArray<FString> FAntigravityReferenceParser::GetAutocompleteSuggestions(const FString& Partial) const
{
...
EAntigravityReferenceType FAntigravityReferenceParser::ClassifyReference(const FString& Ref)
{
	if (Ref.StartsWith(TEXT("/")))
	{
		return EAntigravityReferenceType::File;
	}
	const FString Lower = Ref.ToLower();
	if (Lower == TEXT("errors")) return EAntigravityReferenceType::CompileErrors;
	if (Lower == TEXT("selection")) return EAntigravityReferenceType::Selection;
	if (Lower == TEXT("level")) return EAntigravityReferenceType::Level;
	if (Lower == TEXT("buildlog")) return EAntigravityReferenceType::BuildLog;
	if (Lower.StartsWith(TEXT("asset:"))) return EAntigravityReferenceType::BlueprintAsset;
	return EAntigravityReferenceType::File;
}

bool FAntigravityReferenceParser::IsBinaryFile(const FString& AbsolutePath)
{
...
FString FAntigravityReferenceParser::ResolveAbsolutePath(const FString& RelativePath) const
{
...
FString FAntigravityReferenceParser::BuildFolderSummary(const FString& AbsDir, const FString& RelDir) const
{
	TArray<FString> Files;
...
FString FAntigravityReferenceParser::GatherCompileErrors() const
{
...
FString FAntigravityReferenceParser::GatherSelectionSummary() const
{
	FString Summary;
...
FString FAntigravityReferenceParser::GatherLevelInfo() const
{
#if WITH_EDITOR
...
FString FAntigravityReferenceParser::GatherBuildLog() const
{
