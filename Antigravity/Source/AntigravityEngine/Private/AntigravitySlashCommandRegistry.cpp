...
#include "AntigravitySlashCommandRegistry.h"
#include "Misc/FileHelper.h"
#include "Misc/Paths.h"
#include "HAL/FileManager.h"
#include "Serialization/JsonSerializer.h"

FAntigravitySlashCommandRegistry::FAntigravitySlashCommandRegistry()
{
}
...
void FAntigravitySlashCommandRegistry::Initialize()
{
	RegisterBuiltinCommands();
	LoadCustomCommandsFromDisk();
}

void FAntigravitySlashCommandRegistry::RegisterBuiltinCommands()
{
	// ---- Actor Creation ----
	FAntigravitySlashCommand NewActor;
	...
	NewActor.PromptTemplate =
		TEXT("Create a new Unreal Engine Actor class named '{{arg}}'. Please:\n"
		     ...
		     "Actor name: {{arg}}");
	NewActor.SuggestedMode = EAntigravityAgentMode::CppCode;
	NewActor.bAutoSwitchMode = true;
	Commands.Add(NewActor);

	// ---- Fix Errors ----
	FAntigravitySlashCommand FixErrors;
	...
	FixErrors.PromptTemplate =
		TEXT("Please analyze and fix the current compile errors in this Unreal Engine project.\n\n"
		     ...
		     "@errors");
	FixErrors.SuggestedMode = EAntigravityAgentMode::Debug;
	FixErrors.bAutoSwitchMode = true;
	Commands.Add(FixErrors);

	// ---- Create Material ----
	FAntigravitySlashCommand CreateMaterial;
	...
	CreateMaterial.PromptTemplate =
		TEXT("Create a new Unreal Engine material named '{{arg}}'. Please:\n"
		     ...
		     "Material name: {{arg}}");
	CreateMaterial.SuggestedMode = EAntigravityAgentMode::Asset;
	CreateMaterial.bAutoSwitchMode = true;
	Commands.Add(CreateMaterial);

	// ---- Refactor ----
	FAntigravitySlashCommand Refactor;
	...
	Refactor.PromptTemplate =
		TEXT("Please analyze and refactor the file '{{arg}}'.\n\n"
		     ...
		     "File: @/{{arg}}");
	Refactor.SuggestedMode = EAntigravityAgentMode::CppCode;
	Refactor.bAutoSwitchMode = true;
	Commands.Add(Refactor);

	// ---- Document ----
	FAntigravitySlashCommand Document;
	...
	Document.PromptTemplate =
		TEXT("Please generate comprehensive documentation comments for '{{arg}}'.\n\n"
		     ...
		     "Target: {{arg}}");
	Document.SuggestedMode = EAntigravityAgentMode::Architect;
	Document.bAutoSwitchMode = false;
	Commands.Add(Document);

	// ---- Add Component ----
	FAntigravitySlashCommand AddComponent;
	...
	AddComponent.SuggestedMode = EAntigravityAgentMode::CppCode;
	AddComponent.bAutoSwitchMode = true;
	Commands.Add(AddComponent);

	// ---- Setup Input ----
	FAntigravitySlashCommand SetupInput;
	...
	SetupInput.PromptTemplate =
		TEXT("Set up Enhanced Input for the action '{{arg}}'.\n\n"
		     ...
		     "Action name: {{arg}}");
	SetupInput.SuggestedMode = EAntigravityAgentMode::Blueprint;
	SetupInput.bAutoSwitchMode = false;
	Commands.Add(SetupInput);

	// ---- Create Interface ----
	FAntigravitySlashCommand CreateInterface;
	...
	CreateInterface.PromptTemplate =
		TEXT("Create a Blueprint interface named '{{arg}}'.\n\n"
		     ...
		     "Interface name: {{arg}}");
	CreateInterface.SuggestedMode = EAntigravityAgentMode::Blueprint;
	CreateInterface.bAutoSwitchMode = false;
	Commands.Add(CreateInterface);

	// ---- Setup Replication ----
	FAntigravitySlashCommand SetupReplication;
	...
	SetupReplication.PromptTemplate =
		TEXT("Configure replication for the actor class '{{arg}}'.\n\n"
		     ...
		     "Actor class: {{arg}}");
	SetupReplication.SuggestedMode = EAntigravityAgentMode::CppCode;
	SetupReplication.bAutoSwitchMode = true;
	Commands.Add(SetupReplication);
}

void FAntigravitySlashCommandRegistry::LoadCustomCommandsFromDisk()
{
...
FString FAntigravitySlashCommandRegistry::GetCommandsDirectory()
{
...
bool FAntigravitySlashCommandRegistry::IsSlashCommand(const FString& InputText) const
{
	FString Trimmed = InputText.TrimStart();
	return Trimmed.StartsWith(TEXT("/"));
}

bool FAntigravitySlashCommandRegistry::ExpandSlashCommand(
...
) const
{
	...
	const FAntigravitySlashCommand* Cmd = GetCommand(CommandName);
	if (!Cmd) return false;
	...
	OutSuggestedMode = Cmd->bAutoSwitchMode ? Cmd->SuggestedMode : EAntigravityAgentMode::General;
	return true;
}

TArray<FAntigravitySlashCommand> FAntigravitySlashCommandRegistry::GetSuggestions(const FString& Partial) const
{
	TArray<FAntigravitySlashCommand> Suggestions;
	...
	for (const FAntigravitySlashCommand& Cmd : Commands)
	{
...
void FAntigravitySlashCommandRegistry::RegisterCommand(const FAntigravitySlashCommand& Command)
{
	// Replace if exists
...
const FAntigravitySlashCommand* FAntigravitySlashCommandRegistry::GetCommand(const FString& Name) const
{
	for (const FAntigravitySlashCommand& Cmd : Commands)
	{
