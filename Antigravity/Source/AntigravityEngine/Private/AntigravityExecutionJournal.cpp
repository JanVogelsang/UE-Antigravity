...
#include "AntigravityExecutionJournal.h"
...
FAntigravityExecutionJournal::FAntigravityExecutionJournal()
	: bDirty(false)
{
}
...
void FAntigravityExecutionJournal::RecordExecution(const FAntigravityActionExecutionRecord& Record)
{
	SessionRecords.Add(Record);
	bDirty = true;

	UE_LOG(LogAntigravity, Log, TEXT("ExecutionJournal: Recorded %s â€” %s (success=%s, preHash=%s, postHash=%s)"),
...
void FAntigravityExecutionJournal::FlushToDisk()
{
	...
	for (const FAntigravityActionExecutionRecord& Record : SessionRecords)
	{
	...
	UE_LOG(LogAntigravity, Log, TEXT("ExecutionJournal: Flushed %d records to %s"), SessionRecords.Num(), *FilePath);
}

bool FAntigravityExecutionJournal::LoadFromDisk()
{
	FString FilePath = GetJournalFilePath();
	...
	UE_LOG(LogAntigravity, Log, TEXT("ExecutionJournal: Loaded journal with %d records"),
		RootObj->GetIntegerField(TEXT("record_count")));
	return true;
}

FString FAntigravityExecutionJournal::GetJournalFilePath() const
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("Antigravity"),
		TEXT("ExecutionLog_") + FDateTime::UtcNow().ToString(TEXT("%Y%m%d")) + TEXT(".json"));
}

void FAntigravityExecutionJournal::ClearSession()
{
	SessionRecords.Empty();
	bDirty = false;
}

FString FAntigravityExecutionJournal::BuildRecentActionsSummary(int32 Count) const
{
	...
	{
		const FAntigravityActionExecutionRecord& Record = SessionRecords[i];
...
FString FAntigravityExecutionJournal::ComputeFileHash(const FString& FilePath)
{
...
FString FAntigravityExecutionJournal::ComputeAssetHash(const FString& AssetPath)
{
...
TSharedPtr<FJsonObject> FAntigravityExecutionJournal::RecordToJson(const FAntigravityActionExecutionRecord& Record) const
{
