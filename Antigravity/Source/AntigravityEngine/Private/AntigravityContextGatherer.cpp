...
#include "AntigravityContextGatherer.h"
...
FAntigravityContextGatherer::FAntigravityContextGatherer()
...
/** Directories that must NEVER be iterated â€” they can freeze the editor and blow tokens */
...
class FAntigravityDirectoryVisitor : public IPlatformFile::FDirectoryVisitor
{
public:
	TArray<FAntigravityFileEntry>& Entries;
	FString BasePath;
	TSet<FString> AllowedExtensions;
	int32 MaxEntries;

	FAntigravityDirectoryVisitor(TArray<FAntigravityFileEntry>& InEntries, const FString& InBasePath,
...
FString FAntigravityContextGatherer::BuildContextString()
{
	FAntigravityProjectContext Ctx = BuildProjectContext();
	...
	if (EstTokens > TokenBudget)
	{
		...
		FullContext += TEXT("\n\n[CONTEXT TRUNCATED â€” project has more files/assets than token budget allows. Consider enabling Context-as-Tools for large projects.]");

		UE_LOG(LogAntigravity, Warning, TEXT("ContextGatherer: Context truncated from ~%d to ~%d tokens (budget: %d)"),
...
FString FAntigravityContextGatherer::GetFileTreeString()
{
	FAntigravityProjectContext Ctx = BuildProjectContext();
	FString Result = TEXT("=== Content Directory ===\n");
	for (const FAntigravityFileEntry& Entry : Ctx.ContentTree)
	{
	...
	Result += TEXT("\n=== Source Directory ===\n");
	for (const FAntigravityFileEntry& Entry : Ctx.SourceTree)
	{
		if (Entry.bIsDirectory)
			Result += FString::Printf(TEXT("[DIR] %s\n"), *Entry.Path);
		else
...
FString FAntigravityContextGatherer::GetAssetSummaryString()
{
	FAntigravityProjectContext Ctx = BuildProjectContext();
...
FString FAntigravityContextGatherer::GetSettingsSnapshotString()
{
	return TEXT("=== Settings Snapshot ===\n(Will be populated in Phase 13)");
}

FString FAntigravityContextGatherer::GetClassHierarchyString()
{
...
int32 FAntigravityContextGatherer::EstimateTokenCount() const
{
	return CachedContext.EstimateTokenCount();
}

FAntigravityProjectContext FAntigravityContextGatherer::BuildProjectContext()
{
	...
	FAntigravityProjectContext Ctx;
	...
	// ---- File Trees (with hard-ignore directories) ----
	{
		...
		FAntigravityDirectoryVisitor ContentVisitor(Ctx.ContentTree, FPaths::ProjectContentDir(), ContentExts);
	...
	}
	{
		...
		FAntigravityDirectoryVisitor SourceVisitor(Ctx.SourceTree, FPaths::GameSourceDir(), SourceExts);
	...
	}
	{
		...
		FAntigravityDirectoryVisitor ConfigVisitor(Ctx.ConfigTree, FPaths::ProjectConfigDir(), ConfigExts);
	...
	UE_LOG(LogAntigravity, Log, TEXT("ContextGatherer: Built context â€” %d content files, %d source files, %d assets, %d modules"),
