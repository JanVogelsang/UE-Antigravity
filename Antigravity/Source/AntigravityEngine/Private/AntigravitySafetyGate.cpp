...
void FAntigravitySafetyGate::InitializeDefaults()
{
...
void FAntigravitySafetyGate::LoadProtectedPathsFromSettings()
{
	...
	if (Settings->bOverrideDefaultProtectedPaths)
	{
		...
		UE_LOG(LogAntigravity, Warning,
			TEXT("SafetyGate: bOverrideDefaultProtectedPaths=true â€” default protected paths replaced by user list."));
	}
...
void FAntigravitySafetyGate::InitializeDefaultProtectedPaths()
{
	// Plugin descriptor â€” breaking it kills the plugin
	ProtectedPaths.Add(TEXT("*.uplugin"));

	// Project descriptor â€” editing incorrectly can corrupt the project
	ProtectedPaths.Add(TEXT("*.uproject"));

	// Module build rules â€” incorrect edits prevent compilation
	ProtectedPaths.Add(TEXT("*.Build.cs"));
	ProtectedPaths.Add(TEXT("*.Target.cs"));

	// Core engine config â€” wrong edits can break the engine
	...
	// Antigravity own config
	ProtectedPaths.Add(TEXT(".antigravityignore"));
}

EAntigravityRiskLevel FAntigravitySafetyGate::EvaluateRisk(const FAntigravityAction& Action) const
{
	// Check protected paths â€” always Critical for write operations
	for (const FString& Path : Action.AffectedPaths)
	{
		if (IsPathProtected(Path))
		{
			return EAntigravityRiskLevel::Critical;
		}
	}
	return Action.RiskLevel;
}

bool FAntigravitySafetyGate::IsActionAllowed(const FAntigravityAction& Action, FString& OutReason) const
{
	for (const FString& Path : Action.AffectedPaths)
	{
		...
		{
			OutReason = FString::Printf(TEXT("Path is excluded by .antigravityignore: %s"), *Path);
			return false;
		}
		...
		if (IsPathProtected(Path))
		{
			OutReason = FString::Printf(
				TEXT("Path is protected (read-only for AI): %s\n"
				     "To allow AI modification of this file, remove it from the protected paths list in Antigravity settings."),
				*Path
			);
			return false;
		}
...
bool FAntigravitySafetyGate::IsPathAllowed(const FString& FilePath) const
{
	...
	for (const FString& Allowed : AllowedWritePaths)
	{
		...
		// Ensure we check a full directory boundary â€” path must start with Allowed/ or be exact
...
bool FAntigravitySafetyGate::IsPathProtected(const FString& FilePath) const
{
...
void FAntigravitySafetyGate::AddProtectedPath(const FString& PathOrPattern)
{
	ProtectedPaths.AddUnique(PathOrPattern);
}

void FAntigravitySafetyGate::RemoveProtectedPath(const FString& PathOrPattern)
{
	ProtectedPaths.Remove(PathOrPattern);
}

void FAntigravitySafetyGate::SetIgnoreController(FAntigravityIgnoreController* InController)
{
	IgnoreController = InController;
}

bool FAntigravitySafetyGate::IsPathIgnoredByFilter(const FString& RelativePath) const
{
	if (!IgnoreController)
	{
		return false;
...
bool FAntigravitySafetyGate::ValidateGeneratedCode(const FString& Code, TArray<FString>& OutViolations) const
{
	bool bValid = true;
...
bool FAntigravitySafetyGate::MatchesGlob(const FString& Path, const FString& Pattern) const
{
