...
#include "AntigravityDiffApplicator.h"
#include "Misc/Paths.h"

FAntigravityDiffApplicator::FAntigravityDiffApplicator()
{
}
...
FAntigravityDiffApplyResult FAntigravityDiffApplicator::ApplyDiff(
	const FString& OriginalContent,
	const FString& DiffText,
	float FuzzyThreshold
) const
{
	TArray<FAntigravitySearchReplaceBlock> Blocks;
	if (!ParseDiffBlocks(DiffText, Blocks))
	{
		FAntigravityDiffApplyResult Failure;
...
FAntigravityDiffApplyResult FAntigravityDiffApplicator::ApplyBlocks(
	const FString& OriginalContent,
	const TArray<FAntigravitySearchReplaceBlock>& Blocks,
	float FuzzyThreshold
) const
{
	if (Blocks.Num() == 0)
	...
	// Apply blocks sequentially â€” each block operates on the result of the previous
	FString RunningContent = OriginalContent;

	for (int32 i = 0; i < Blocks.Num(); i++)
	{
		FAntigravityDiffApplyResult BlockResult = ApplySingleBlock(RunningContent, Blocks[i], FuzzyThreshold);
	...
	FAntigravityDiffApplyResult Success;
...
bool FAntigravityDiffApplicator::ParseDiffBlocks(const FString& DiffText, TArray<FAntigravitySearchReplaceBlock>& OutBlocks)
{
	OutBlocks.Empty();
	...
	FAntigravitySearchReplaceBlock CurrentBlock;
	...
	for (const FString& Line : Lines)
	{
		...
		if (State == EParseState::Outside)
		{
			if (TrimmedLine == SearchMarker)
			{
				...
				SearchLines.Empty();
				ReplaceLines.Empty();
				CurrentBlock = FAntigravitySearchReplaceBlock();
			}
		}
		else if (State == EParseState::InSearch)
		{
			...
			{
				// Separator between hint and content â€” skip
			}
			else
			{
...
FAntigravityDiffApplyResult FAntigravityDiffApplicator::ApplySingleBlock(
	FString& Content,
	const FAntigravitySearchReplaceBlock& Block,
	float FuzzyThreshold
) const
{
	FAntigravityDiffApplyResult Result;
	...
	if (ExactIdx != INDEX_NONE)
	{
		...
		{
			// Normalized match but not exact â€” fall back to normalized replacement
			ActualIdx = ExactIdx;
	...
	// Exact match failed â€” try fuzzy matching
	TArray<FString> Lines;
	Content.ParseIntoArrayLines(Lines, false);
...
bool FAntigravityDiffApplicator::FindBestMatch(
...
float FAntigravityDiffApplicator::ComputeSimilarity(const FString& A, const FString& B)
{
...
int32 FAntigravityDiffApplicator::LevenshteinDistance(const FString& A, const FString& B)
{
...
FString FAntigravityDiffApplicator::NormalizeForComparison(const FString& Text)
{
	FString Result = Text;
