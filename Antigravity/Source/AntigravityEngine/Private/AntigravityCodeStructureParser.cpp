...
#include "AntigravityCodeStructureParser.h"
#include "Misc/FileHelper.h"
#include "Misc/Paths.h"
#include "HAL/FileManager.h"

FAntigravityCodeStructureParser::FAntigravityCodeStructureParser()
{
}

// ============================================================================
// FAntigravityFileStructure
// ============================================================================

FString FAntigravityFileStructure::ToFoldedString() const
{
	...
	for (const FAntigravityCodeDeclaration& Decl : Declarations)
	{
...
// ============================================================================
// FAntigravityCodeStructureParser
// ============================================================================

bool FAntigravityCodeStructureParser::IsSupportedFileType(const FString& FilePath)
{
	FString Ext = FPaths::GetExtension(FilePath).ToLower();
	return Ext == TEXT("h") || Ext == TEXT("cpp") || Ext == TEXT("cs");
}

FAntigravityFileStructure FAntigravityCodeStructureParser::ParseFile(
	const FString& AbsolutePath,
	const FString& RelativePath) const
{
	FAntigravityFileStructure Result;
...
FAntigravityFileStructure FAntigravityCodeStructureParser::ParseCppFile(
	...
	const FString& RelativePath) const
{
	...
	for (int32 i = 0; i < Lines.Num(); i++)
	{
		...
		{
			FAntigravityCodeDeclaration Decl;
...
FString FAntigravityCodeStructureParser::GenerateFoldedContext(
	...
	int32 MaxCharacters) const
{
	...
	for (const TPair<FString, FString>& FilePair : FilePaths)
	{
		...
		FAntigravityFileStructure Structure = ParseFile(AbsPath, RelPath);
...
bool FAntigravityCodeStructureParser::IsUEMacroLine(const FString& Line)
{
...
bool FAntigravityCodeStructureParser::IsClassDeclarationLine(const FString& Line)
{
...
bool FAntigravityCodeStructureParser::IsUPropertyLine(const FString& Line)
{
	return Line.StartsWith(TEXT("UPROPERTY(")) || Line.StartsWith(TEXT("UPROPERTY()"));
}

bool FAntigravityCodeStructureParser::IsUFunctionLine(const FString& Line)
{
	return Line.StartsWith(TEXT("UFUNCTION(")) || Line.StartsWith(TEXT("UFUNCTION()"));
}

bool FAntigravityCodeStructureParser::IsFunctionDeclarationLine(const FString& Line)
{
...
FString FAntigravityCodeStructureParser::ExtractFunctionSignature(const FString& Line) const
{
...
FString FAntigravityCodeStructureParser::TruncateDeclaration(const FString& Line) const
{
