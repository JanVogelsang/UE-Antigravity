...
FAgentFrameworkActionResult FAgentFrameworkInputActions::ExecuteAddInputMapping(const TSharedRef<FJsonObject>& Params)
{
	...
	{
		Mapping.Triggers.Add(NewObject<UInputTriggerPressed>());
		UE_LOG(LogAgentFramework, Log, TEXT("InputActions: No triggers specified â€” defaulting to Pressed trigger."));
	}
	...
	UE_LOG(LogAgentFramework, Log, TEXT("InputActions: Added mapping %s â†’ %s (Key=%s)"),
