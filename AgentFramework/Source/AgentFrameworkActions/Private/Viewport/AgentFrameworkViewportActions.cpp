	...
	int32 JpegQuality)
{
	...
	UE_LOG(LogAgentFramework, Log, TEXT("ViewportActions: JPEG encoded %dx%d Q%d -> %d bytes (vs PNG would be ~%dx larger)"),
...
FString FAgentFrameworkViewportActions::EncodePixelsToBase64(
	const TArray<FColor>& Pixels,
	int32 Width,
	int32 Height,
	int32 MaxDimension,
	int32 JpegQuality)
{
	...
	if (!ImageWrapper.IsValid())
	{
		UE_LOG(LogAgentFramework, Error, TEXT("ViewportActions: Failed to create JPEG ImageWrapper for Base64 encoding"));
		return FString();
	}
	...
	if (!ImageWrapper->SetRaw(RawData.GetData(), RawData.Num(), OutWidth, OutHeight, ERGBFormat::BGRA, 8))
	{
		UE_LOG(LogAgentFramework, Error, TEXT("ViewportActions: Failed to set raw pixel data on ImageWrapper for Base64 encoding"));
		return FString();
	}
	...
	if (CompressedData.Num() == 0)
	{
		UE_LOG(LogAgentFramework, Error, TEXT("ViewportActions: JPEG compression returned empty data for Base64 encoding"));
		return FString();
	}
