...
bool FAgentFrameworkSourceControlActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const { return true; }
...
FAgentFrameworkActionResult FAgentFrameworkSourceControlActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	...
	{
		FString ToolName;
		if (Params->TryGetStringField(TEXT("tool_name"), ToolName) || Params->TryGetStringField(TEXT("_tool_name"), ToolName))
		{
	...
	if (Params->TryGetArrayField(TEXT("file_paths"), PathsArray))
	{
	...
	else if (Params->TryGetArrayField(TEXT("files"), PathsArray))
	{
		for (const auto& Val : *PathsArray)
		{
			FString Path;
