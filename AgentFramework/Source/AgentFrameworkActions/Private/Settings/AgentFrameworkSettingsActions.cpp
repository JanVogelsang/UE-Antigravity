...
FAgentFrameworkSettingsActions::~FAgentFrameworkSettingsActions() {}
FName FAgentFrameworkSettingsActions::GetActionName() const { return FName(TEXT("Settings")); }
...
bool FAgentFrameworkSettingsActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
	...
	{
		if (!Params->HasField(TEXT("section")) || !Params->HasField(TEXT("key")))
		{
			OutErrors.Add(TEXT("Missing required field(s) for read_config_value: section, key"));
			return false;
		}
	}
	else if (ToolName == TEXT("write_config_value"))
	{
		if (!Params->HasField(TEXT("section")) || !Params->HasField(TEXT("key")) || !Params->HasField(TEXT("value")))
		{
			OutErrors.Add(TEXT("Missing required field(s) for write_config_value: section, key, value"));
			return false;
		}
	}
	else if (ToolName == TEXT("macro_ensure_project_prerequisites"))
	{
		if (!Params->HasField(TEXT("plugins")))
		{
			OutErrors.Add(TEXT("Missing required field for macro_ensure_project_prerequisites: plugins (must be an array)"));
			return false;
		}
	}
...
FAgentFrameworkActionResult FAgentFrameworkSettingsActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	...
	// Determine if read-only or doesn't support transactions
	bool bIsWrite = Params->HasField(TEXT("value")) || ToolName == TEXT("write_config_value");
	bool bNeedsTransaction = bIsWrite && (ToolName != TEXT("macro_ensure_project_prerequisites"));
	...
	{
		Result = ExecuteMacroEnsureProjectPrerequisites(Params, Result);
		return Result;
	}
	...
	else
	{
		...
		if (!bUsedReflection)
		{
			...
			UE_LOG(LogAgentFramework, Log, TEXT("SettingsActions: Set [%s] %s = %s via GConfig (no live UI update)"),
				*Section, *Key, *Value);
		}
...
FAgentFrameworkActionResult FAgentFrameworkSettingsActions::ExecuteMacroEnsureProjectPrerequisites(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result)
{
