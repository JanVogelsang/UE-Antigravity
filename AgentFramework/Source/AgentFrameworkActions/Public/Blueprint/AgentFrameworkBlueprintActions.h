...
/**
 * FAgentFrameworkBlueprintActions
 *
 ...
 */
class AGENTFRAMEWORKACTIONS_API FAgentFrameworkBlueprintActions : public IAgentFrameworkActionExecutor
{
public:
	FAgentFrameworkBlueprintActions();
	virtual ~FAgentFrameworkBlueprintActions();
...
private:
	...
	FAgentFrameworkActionResult ExecuteCreateBlueprint(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);

	/** Add a component to an existing Blueprint via the Simple Construction Script. */
	FAgentFrameworkActionResult ExecuteAddComponent(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);

	/** Add a typed member variable to a Blueprint. */
	FAgentFrameworkActionResult ExecuteAddVariable(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);

	/** Compile a Blueprint and return all errors/warnings. */
	FAgentFrameworkActionResult ExecuteCompileBlueprint(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);

	/** Set default property values on a Blueprint's Class Default Object. */
	FAgentFrameworkActionResult ExecuteSetDefaults(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);

	FAgentFrameworkActionResult ExecuteExportBlueprintSummary(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteInjectNodesT3D(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteAddFunction(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteSetComponentProperties(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 *   - asset_path (string, required): Content path of the Blueprint
	 */
	FAgentFrameworkActionResult ExecuteGetBlueprintInfo(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteAnalyzeBlueprintGraph(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteAddEventHandler(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteConnectPins(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteAddEnhancedInputNode(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteVerifyConnections(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 */
	FAgentFrameworkActionResult ExecuteSetNodePinDefault(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);

	/**
	 * Execute sequential operations on an asset in a single transaction and compile once at the end.
	 *
	 * Parameters:
	 *   - asset_path (string, required): Content path of the Blueprint
	 *   - operations (array of objects, required): Array of operations containing tool_name and arguments
	 */
	FAgentFrameworkActionResult ExecuteBatchOperations(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
	 ...
	 *   - graph_name (string, optional): Target graph. Default: "EventGraph".
	 */
	FAgentFrameworkActionResult ExecuteDeleteNodes(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);

	/** Extract Blueprint schema without loading the asset in memory. */
	FAgentFrameworkActionResult ExecuteGetBlueprintSchema(const TSharedRef<FJsonObject>& Params, FAgentFrameworkActionResult& Result);
...
private:
	 ...
	 */
	static bool CompileAndReport(UBlueprint* Blueprint, FAgentFrameworkActionResult& Result, bool bSkipGC = true);

	/** Build a JSON string describing variables, SCS components, and graph inventory. */
	static FString BuildBlueprintInfoJson(UBlueprint* Blueprint, const TArray<FString>* NodeNamesFilter = nullptr, bool bExcludeVisualLayout = false, const FString& QueryMode = TEXT("full"));

	/** Shared helper to connect pins between two nodes in a graph. */
	static bool ConnectPinsHelper(
	...
	/** Static tracking map for asset modifications to support client-side hashes. */
	static TMap<FString, int32> AssetModificationCounts;
