...
void FAntigravitySkillsManager::Initialize()
{
	...
	TSharedPtr<IPlugin> AntigravityPlugin = IPluginManager::Get().FindPlugin(TEXT("Antigravity"));
	FString PluginBaseDir = AntigravityPlugin.IsValid() ? AntigravityPlugin->GetBaseDir() : FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("Antigravity"));
