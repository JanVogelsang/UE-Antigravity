...
bool FAntigravityPCGActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
	return true;
}
...
bool FAntigravityPCGActions::CheckPCGAvailable(FAntigravityActionResult& Result)
{
...
AActor* FAntigravityPCGActions::FindActorByName(const FString& ActorName)
{
#if WITH_EDITOR
...
FAntigravityActionResult FAntigravityPCGActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	FScopedTransaction Transaction(FText::FromString(TEXT("Antigravity PCG Action")));
...
FAntigravityActionResult FAntigravityPCGActions::ExecuteCreatePCGGraph(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	...
	UE_LOG(LogAntigravity, Log, TEXT("PCGActions: Created PCG graph at '%s'"), *AssetPath);
...
FAntigravityActionResult FAntigravityPCGActions::ExecuteAttachPCGComponent(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	if (!CheckPCGAvailable(Result)) return Result;
	...
	if (!ExistingComp)
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("PCGActions: Added PCGComponent to actor '%s'"), *ActorName);
	}
	else
	{
...
FAntigravityActionResult FAntigravityPCGActions::ExecuteSetPCGParameter(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	if (!CheckPCGAvailable(Result)) return Result;
...
FAntigravityActionResult FAntigravityPCGActions::ExecuteGeneratePCGLocal(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	if (!CheckPCGAvailable(Result)) return Result;
	...
	if (GenerateLocalFunc)
	{
		...
		UE_LOG(LogAntigravity, Log, TEXT("PCGActions: Called GenerateLocal(force=%s) on '%s'"), bForce ? TEXT("true") : TEXT("false"), *ActorName);
	}
	else
	{
...
FAntigravityActionResult FAntigravityPCGActions::ExecuteGetPCGInfo(const TSharedRef<FJsonObject>& Params, FAntigravityActionResult& Result)
{
	if (!CheckPCGAvailable(Result)) return Result;
