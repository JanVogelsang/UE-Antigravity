...
bool FAntigravityLevelActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
	FString ToolName;
...
{
	FScopedTransaction Transaction(FText::FromString(TEXT("Antigravity Level Action")));
	...
	if (ToolName == TEXT("spawn_actor"))
	{
		Result = ExecuteSpawnActor(Params, World);
	}
	else if (ToolName == TEXT("place_light"))
	{
		Result = ExecutePlaceLight(Params, World);
	}
	else if (ToolName == TEXT("modify_world_settings"))
	{
		Result = ExecuteModifyWorldSettings(Params, World);
	}
	else
	{
...
FAntigravityActionResult FAntigravityLevelActions::ExecuteSpawnActor(const TSharedRef<FJsonObject>& Params, UWorld* World)
{
	FAntigravityActionResult Result;
	Result.bSuccess = false;
...
FAntigravityActionResult FAntigravityLevelActions::ExecutePlaceLight(const TSharedRef<FJsonObject>& Params, UWorld* World)
{
	FAntigravityActionResult Result;
	Result.bSuccess = false;
...
FAntigravityActionResult FAntigravityLevelActions::ExecuteModifyWorldSettings(const TSharedRef<FJsonObject>& Params, UWorld* World)
{
	FAntigravityActionResult Result;
	Result.bSuccess = false;
