## 2026-07-17T18:36:53Z
You are the Cpp Refactoring Worker (worker_cpp).
Your working directory is C:\Users\janv1\Documents\Unreal Projects\UE-Antigravity\.agents\worker_cpp.
Your role is to refactor the Cpp action module in the UE-Antigravity plugin.
...
Your specific tasks:
1. Consolidate JSON parsing in `AgentFrameworkCppActions.cpp` using static helpers from `UAgentFrameworkActionUtils` (found in `AgentFrameworkActionUtils.h/cpp`). Specifically, replace raw `GetStringField`, `TryGetStringField`, `GetNumberField`, `TryGetNumberField`, `GetArrayField`, `TryGetArrayField`, etc. with the safe helper methods where applicable, or expand `UAgentFrameworkActionUtils` with new static helpers if needed and use them.
2. Clean up `AgentFrameworkCppActions.h` and `AgentFrameworkCppActions.cpp` by deleting orphaned helper functions, unused includes, and dead code.
3. Implement strict null-checking (`IsValid()`) for all Unreal Engine object pointers in these files to prevent Editor crashes. (Note: Only use `IsValid` on actual Unreal Object pointers inheriting from UObject, such as UClass*, UPackage*, UFunction*, etc. Do not use it on non-UObjects like IDesktopPlatform* or FProperty*).
...
6. Write a handoff report at `.agents/worker_cpp/handoff.md` detailing changes, compilation output, test results, and verified files.
